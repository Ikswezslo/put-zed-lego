...
Z tego co widzimy na wykresie gęstości, najwięcej zestawów składa się tylko z jednej części. W miarę zwiększania się liczby części, wartość funkcji gęstości maleje. Wyjątkiem tutaj są zestawy z około 100 częściami - tutaj następuje lekkie zwiększenie wartości co oznacza, że powstaje stosunkowo dużo zestawów takich zestawów. Takie podwyższenie można zaobserwować róznież w okolicach liczby 200, lecz jest to znacznie mniej zauważalne.
...
Powyższy wykres został ograniczony jedynie do zestawów, które posiadają przynajmniej jedną minifigurkę.
...
Zatem mniej więcej co 3 zestaw zawiera przynajmniej jedną minifigurkę.
...
Przeważająca większość zestawów posiada tylko jedną wersją, lecz mimo to zauważa
Dość znaczna ilość zestawów powstaje w więcej niż jednej wersji.
...
Widzimy od końca lat 90 gwałtowny wzrost produkowanych zestawów, a następnie spadek od roku 2002 do 2005. Prawdopodobnie jest to powiązane z kryzysem firmy w tamtym czasie i drastycznymi cięciami kosztów: https://businessinsider.com.pl/firmy/strategie/lego-model-biznesowy-i-strategia-vidiyo-zestawy-hidden-side-nowe-produkty-i-historia/4zzgq1x
...
lego_sets %>% 
  ...
  ggplot(aes(x=year, y=mean_minifigs)) +
    ...
    ggtitle("Średnia liczba figurek w zestawach Lego na przestrzeni lat") + 
    xlab("rok") + 
    ylab("średnia liczba figurek") +
    theme_minimal()
```

Pierwsze minifigurki powstały około 1975 roku i od tego czasu ich średnia liczba w zestawach nie ma trendu rosnącego ani spadkowego co widać wyraźniej na wykresie poniżej uwzględniającym jedynie zestawy z przynajmniej jedną minifigurką:
...
Wraz w powstaniem minifigurek Lego, ilość zestawów z ich udziałem rosła/malała podobnie jak zestawów bez minifigurek.
...
lego_sets_with_price %>% 
  ggplot(aes(x=price, y=num_parts)) +
    geom_point(alpha = 1/10) + 
    geom_smooth(method = lm) + 
    ggtitle("Zależność między ceną a liczbą części") + 
    xlab("liczba części") + 
    ylab("cena") +
    theme_minimal()
```

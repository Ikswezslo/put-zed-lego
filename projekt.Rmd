...
Z wykresu ciężko coś odczytać ze względu na dużą liczbę ekstremalnych wartości oraz skoncentrowanie danych w okolicach zera. Z tego względu postanowiłem pozbyć się wartości najbardziej odstających: 
...
Kolejne 2 wykresy przedstawiają rozkład liczby części Lego po usunięciu ekstremalnych wartości: 

```{r}
p <- ggplot(lego_sets_num_parts_no_outliers, aes(x=num_parts)) +
  geom_density() + 
  ggtitle("Rozkład liczby części Lego bez ekstremalnych wartości") + 
  xlab("liczba części") + 
  ylab("") +
  theme_minimal() + 
  theme(axis.text.y=element_blank(),
...
lego_sets %>% 
  ...
  ggplot(aes(x=num_parts, y=minifigs_count)) +
    ...
    geom_smooth(method = lm) + 
    ggtitle("Zależność między liczbą minifigurek a liczbą części") + 
    xlab("liczba części") + 
    ylab("liczba minifigurek") +
    theme_minimal()
```
...
Początkowo odbywa się faza dostrajania hiperparametru 'nrounds' oznaczającego liczbę iteracji w trakcie treningu. Poniżej znajduje się wykres z uzyskanymi wynikami i wybraną liczbą iteracji oznaczoną niebieską linią:
